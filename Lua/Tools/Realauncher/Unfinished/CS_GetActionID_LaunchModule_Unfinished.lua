function getActionIDNumber(arguments)
	if arguments and string.len(arguments) >= 1 then
		...
		local matches = searchMatrixAndReturnMatchesTable(action,"name",realauncher.argumentElement)
		...
		if char == 13 and currentAction then reaper.ShowMessageBox(currentAction.id,"Action command ID",0) 
		end
	end
	
end	

registeredCommands.cmdID = {getActionIDNumber, runImediatelly}